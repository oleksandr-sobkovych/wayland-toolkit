...
use wayland_client::{protocol::{wl_surface, wl_shm_pool, wl_shm, wl_buffer}, Main};
...
pub struct SurfaceObj {
    ...
    pub can_change: bool,
    pub grad_offset: u8,
    pub pool: Main<wl_shm_pool::WlShmPool>,
    pub tmp: File,
    pub width: i32,
    pub height: i32
}
...
pub fn draw_window(buf_x: i32, buf_y: i32, mut tmp: &File, grad_offset: u8) {
    tmp.seek(SeekFrom::Start(0)).unwrap();
    let mut buf = BufWriter::new(&mut tmp);
    for _ in 0..buf_y {
        for _ in 0..buf_x {
            let color: i32 = 0xFF + 0xFF << (8*grad_offset);
...
pub fn redraw_surface(mut surface_obj: SurfaceObj) -> SurfaceObj {
    draw_window(surface_obj.width, surface_obj.height, &surface_obj.tmp, surface_obj.grad_offset);
